...
console.log(allPlayerAnnouncement)
let port;
let system;
...
function sayAll(player, values) {
  ...
  return tryDownloadTTS(text, language)
    .then((result) => {
      return allPlayerAnnouncement.announceAll(player.system, `http://${player.system.localEndpoint}:${port}${result.uri}`, announceVolume, result.duration);
    })
