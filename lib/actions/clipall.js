...
const {announceAll} = require('../helpers/all-player-announcement');
...
function playClipOnAll(player, values) {
    ...
    return fileDuration(path.join(LOCAL_PATH_LOCATION, clipFileName))
        .then((duration) => {
            return announceAll(player.system, `http://${player.system.localEndpoint}:${port}/clips/${clipFileName}`, announceVolume, duration);
        });
}
