...
const { getTtsFilePath, getTtsUri } = require('../helpers/tts-paths');
...
function eleven(phrase, voiceId) {
  console.log(`Elevenlabs TTS requested for phrase "${phrase}" with voiceId "${voiceId}"`);
  if (!settings.elevenlabs) {
    console.log('Elevenlabs TTS not enabled in settings!');
  ...
  const filepath = getTtsFilePath(settings, filename);

  synthesizeParameters.fileName = filepath;

  const expectedUri = getTtsUri(filename);
  try {
