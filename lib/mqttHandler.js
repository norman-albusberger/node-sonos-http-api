...
function setupMQTT(discovery, settings) {
    const broker = settings.mqtt.broker || settings.mqtt.host || 'localhost';
    const brokerUrl = broker.startsWith('mqtt://') ? broker : `mqtt://${broker}`;
    ...
    mqttClient = mqtt.connect(brokerUrl, {
        ...
        clientId: `sonox-${Math.random().toString(16).slice(2, 10)}`,
        clean: true,
