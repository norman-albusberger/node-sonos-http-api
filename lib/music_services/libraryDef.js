...
const randomQueueLimit = (settings.library && settings.library.randomQueueLimit !== undefined) ? settings.library.randomQueueLimit : 50;
    ...
    if (isFinished(chunk)) {
      return new Promise((resolve, reject) => {
        fs.writeFile(libraryPath, JSON.stringify(library), (err) => {
          isLoading = false;
          if (err) {
            logger.error('Error writing library cache', err);
            return reject(err);
          } else {
...
function searchLibrary(type, term) {
  ...
  return (type === 'album') ? fuzzyAlbums.search(term) : shuffleArray(fuzzyTracks.search(term)).slice(0, randomQueueLimit);
}
