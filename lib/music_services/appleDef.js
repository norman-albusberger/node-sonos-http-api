...
function getURI(type, id, context) {
  ...
  if (type == 'song') {
    return `x-sonos-http:song%3a${id}.mp4?sid=${context.sid}&flags=8224&sn=${context.accountSN}`;
  } else
  if (type == 'station') {
    return `x-sonosapi-radio:radio%3ara.${id}?sid=${context.sid}&flags=8300&sn=${context.accountSN}`;
  }
}
...
function getURIandMetadata(type, resList, context)
{
  ...
  UaM.metadata = getMetadata(type, MetadataID, Name, Title, context);
    ...
    tracks.queueTracks = tracksJson.results.reduce(function(tracksArray, track) {
      if (track.isStreamable) {
        ...
        if (!skip) {
          ...
          var metadata = getMetadata('song', metadataID, track.trackId, track.trackName, context);
          var uri = getURI('song', encodeURIComponent(track.trackId), context);
