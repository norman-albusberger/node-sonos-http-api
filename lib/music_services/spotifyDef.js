...
function getURI(type, id, context) {
  ...
  if (type == 'song') {
    return `x-sonos-spotify:spotify%3atrack%3a${id}?sid=${context.sid}&flags=8224&sn=${context.accountSN}`;
  } else
  if (type == 'station') {
    return `x-sonosapi-radio:spotify%3aartistRadio%3a${id}?sid=${context.sid}&flags=8300&sn=${context.accountSN}`;
  } else
  if (type == 'playlist') {
...
function getServiceToken(context) {
  return `SA_RINCON${context.serviceType}_X_#Svc${context.serviceType}-0-Token`;
}
...
function getURIandMetadata(type, resList, context)
{
  ...
  UaM.metadata = getMetadata(type, MetadataID, (type=='album' || type=='playlist')?Title.toLowerCase() : Id, Title, context);
  UaM.uri = getURI(type, encodeURIComponent((type=='station')?items[0].id:items[0].uri), context);
...
function loadTracks(type, tracksJson, context)
{
  ...
  if (tracksJson.tracks.items.length > 0) {
    ...
    tracks.queueTracks = tracksJson.tracks.items.reduce(function(tracksArray, track) {
      if (track.available_markets == null || track.available_markets.indexOf(context.country) != -1) {
        var skip = false;
        ...
        if (!skip) {
          ...
          var metadata = getMetadata('song', metadataID, track.id, track.name, context);
