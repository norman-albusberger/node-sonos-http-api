...
This repository is being aligned to Node 24 LTS without changing the existing HTTP API contract.
...
- The fork now runs in practice on Node 24, but it still contains older implementation patterns in secondary paths and legacy tests.
...
2. Add a minimal compatibility test matrix for Node 24 against the fork.
...
Target runtime for this repository: Node 24 LTS with no breaking changes to existing endpoints.
