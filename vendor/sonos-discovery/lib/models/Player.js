...
function Player(data, listener, system) {
  ...
  const uri = new URL(data.location);
  _this.baseUrl = uri.origin;
...
Object.setPrototypeOf(Player, EventEmitter);
...
Player.prototype.browseAll = async function browseAll(objectId) {
  const result = {
    items: [],
    startIndex: 0,
    numberReturned: 0,
  ...
  while (true) {
    if (!chunk || !Array.isArray(chunk.items) || Number.isNaN(chunk.totalMatches)) {
    ...
    chunk = await this.browse(objectId, chunk.startIndex + chunk.numberReturned, 0);
  }
};
